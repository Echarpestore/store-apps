...
t('Local-agent sale_saved does not generate stored evidence video',pos.includes("c.localOnly&&meta.type==='sale_saved')return true"));
t('Glow metadata marks NVR on demand',pos.includes("doc.video='nvr_on_demand'"));
t('POS cache bust v495+',/cctv-invoice\.js\?v=(?:49[5-9]|[5-9]\d{2,})/.test(pidx));
...
t('Invoice video explains fast 480p default',off.includes('480p افتراضيًا للسرعة'));
t('Invoice video still offers 720p',off.includes('720p أوضح'));
...
t('Office cache bust v495+',/cctv\.js\?v=(?:49[5-9]|[5-9][0-9]{2,})/.test(oidx));
