...
(async () => {
  ...
  await t('🔴 تابلت الفرع مبيمسحش كاش POS/الحضور/المكتب', async () => {
    const s = await activate('feedback/sw.js', ['pos-shell-v730', ...others, 'store-apps-shell-v703']);
    ok(s.has('pos-shell-v730') && others.every(n => s.has(n)), 'اتمسح: ' + ['pos-shell-v730', ...others].filter(n => !s.has(n)));
