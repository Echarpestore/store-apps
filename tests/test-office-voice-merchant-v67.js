...
assert(src.includes("batch.set(ref,{merchantId:d.merchant.id,type:'order'"),'order write only in commit');
assert(src.includes("batch.set(ref2,{merchantId:d.merchant.id,type:'payment'"),'payment write in same batch');
...
assert(src.includes("if(m.after<0)"),'overpayment warning');
...
assert(src.includes("قول الاسم كامل زي ما هو مسجل"),'ambiguous name asks for exact registered name');
