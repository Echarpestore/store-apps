...
const THU = '2026-08-06', FRI = '2026-08-07', SAT = '2026-08-08', SUN = '2026-08-09', MON='2026-08-10', TUE='2026-08-11';
...
// ============================================================
(function(){
  assertEq(ofSettleDayFor(THU, {}), '2026-08-11', '⭐⭐ فيزا الخميس ضمن أسبوع ينزل الثلاثاء');
  assertEq(ofSettleDayFor(SUN, {}), '2026-08-11', 'وفيزا الأحد بتنزل الثلاثاء');
  // الفرع فاتح الجمعة، بس البنك لأ — فأول يوم عمل بعد الجمعة هو الأحد،
  // يعني مبيعات الخميس والجمعة بينزلوا **مع بعض** يوم الأحد.
  assertEq(ofSettleDayFor(FRI, {}), TUE,
    '⭐ وفيزا الجمعة ضمن نفس تحويل الثلاثاء');
  assertEq(ofSettleDayFor(SAT, {}), '2026-08-11', 'وفيزا السبت برضه الثلاثاء');
  // إعداد مختلف: تأخير يومين عمل
  assertEq(ofSettleDayFor('2026-08-11', {}), '2026-08-18', 'بيع الثلاثاء يدخل تحويل الثلاثاء اللي بعده');
})();
...
// ============================================================
// ٤) 🔮 التوقّع الأسبوعي بينزل الثلاثاء وبالصافي
// ============================================================
(function(){
  ...
  assertEq(rowOf(L, SUN).pmExpected, 0, 'ولا الأحد');
  const tue = rowOf(L, TUE);
  assert(tue.pmExpected > 0, '⭐⭐ المتوقّع بينزل الثلاثاء');
  assertEq(tue.pmExpected, 9806, 'وبالصافي بعد ١.٩٤% (١٠٠٠٠ − ١٩٤)');
  assertEq(tue.pmFrom[0], THU, 'ومكتوب جاي من مبيعات أنهي يوم');
  assertEq(tue.balance, 0, '⭐⭐ الرصيد المؤكد صفر — الفلوس لسه مااتأكدتش');
  assertEq(tue.balanceExp, 9806, 'والرصيد المتوقّع فيه الفلوس');
})();
...
(function(){
  ...
  const data = {
    ...
    // التحويل الأسبوعي اتأكد الثلاثاء عن الأسبوع المنتهي الاثنين
    settlements: [ { ts: at(2026,8,11,13), net: 9800, gross: 10000, forDay: MON, weeklyCycleEnd:MON } ]
  };
  ...
  assertEq(tue.balance, 9800, '⭐⭐ الرصيد = التحويل الحقيقي مرة واحدة بالظبط');
  assertEq(tue.balanceExp, 9800, 'والمتوقّع بنفس الرقم — مفيش زيادة وهمية');

  // 🔒 بعد تأكيد الأسبوع مفيش توقّع لنفس المبيعات
  const after = L.rows.filter(r => r.key >= TUE && r.pmExpected > 0);
  assertEq(after.length, 0, '🔒 مفيش أي توقّع بعد ما الرصيد المستحق خلص');
  assertEq(L.outstanding, 0, 'والمستحق عند Paymob بقى صفر');
})();

// ============================================================
// ٦) 🔒 تحويل يدوي غامض مايقفلش أسبوع من غير cycleEnd/forDay مناسب
// ============================================================
(function(){
  const base = { amount: 0, atMs: at(2026, 8, 6, 7) };
  const data = {
  ...
  assertEq(L.outstanding, 1000, 'التحويل الغامض مايقفلش مبيعات الأسبوع بالغلط');
  const totalPred = L.rows.reduce((a, r) => a + r.pmExpected, 0);
  assertEq(totalPred, 980, 'والتوقّع الأسبوعي يفضل موجود لحد التأكيد بالنسبة المتعلّمة من التحويل اليدوي');
})();
...
// ============================================================
// ١٦) 📈 النسبة الفعلية بتتعلّم من التحويلات الأسبوعية الحقيقية
// ============================================================
(function(){
  const base = { amount: 0, atMs: at(2026, 8, 3, 7) };
  const data = {
    sales: [ sale(at(2026,8,3,15), 0, 5000), sale(at(2026,8,9,15), 0, 10000) ],
    settlements: [ { ts: at(2026,8,4,13), net: 4911.85, gross: 5000,
                     forDay:'2026-08-03', weeklyCycleEnd:'2026-08-03' } ]
  };
  ...
  const tue = rowOf(L, TUE);
  assertEq(tue.pmExpected, 9824, 'والتوقّع الأسبوعي اتحسب بالنسبة المتعلّمة');
})();

// ============================================================
// ١٦.٥) 📦 دفعات التجار الجديدة تخصم السيولة بدون مضاعفة التاريخ القديم
// ============================================================
(function(){
  const base={amount:10000,atMs:at(2026,8,10,7)};
  const oldPay={ts:at(2026,8,10,12),amount:2000,type:'payment'}; // قبل v65
  const newPay={ts:at(2026,8,10,13),amount:1500,type:'payment',cashTracked:true};
  const L=ofCashLedger(base,{sales:[],mtxns:[oldPay,newPay]}, {}, {predict:false}, at(2026,8,10,20),0);
  const r=rowOf(L,MON);
  assertEq(r.val.supplierPayments,1500,'بس الدفعة الجديدة المعلّمة بتدخل الدفتر');
  assertEq(r.balance,8500,'السيولة نقصت مرة واحدة من الدفعة الجديدة');
})();
