...
(function (root) {
  ...
  /* ============================================================
     ...
     بالظبط (نصوص قصيرة، من غير محارف غريبة). لازم يتطابق مع
     السيرفر: لو العميل نضّف بطريقة مختلفة، ممكن يبعت ألوان متقبلش
     هناك أو العكس. القص بعدين (sliceGridProportional) بيعتمد على
     إن العدد هنا نفس العدد اللي السيرفر رجّعه.

     🔴🔴🔴🔴⭐ قرار معماري نهائي: حد أقصى ٣ ألوان (٤ خانات كحد
     أقصى مع "بدون بندانة") — نداء واحد بس، هندسة بسيطة يمكن
     تصحيحها لو الموديل قلب الاتجاه (resolveGridOrientation).
     شبكات أكبر جرّبناها وبتتلخبط (زوم مبالغ فيه + فهرسة مش
     مضمونة) — المعامل ده مكرر في ٣ أماكن تانية (chat-staff-ui.js
     CC_BAND_MAX، وتنضيف الألوان في loyalty/index.html وglow/index.html)
     وأي تغيير هنا لازم يتغيّر في الأربعة مع بعض. */
  var MAX_BANDANA_COLORS = 3;
  ...
  /* 🔍 بصمة صورة العميلة — بتحدد إذا كانت "نفس الصورة" في الكاش.
     🔴🔴🔴🔴⭐ الإصدار القديم كان بياخد ٤ عيّنات × ١٢ حرف من أماكن
     نسبية ثابتة (٢٠٪/٤٠٪/٦٠٪/٨٠٪) + الطول. المشكلة الحقيقية اللي
     ظهرت في الاستخدام: صورتين مختلفتين متصوّرتين بنفس الموبايل في
     نفس الإضاءة بيطلعوا بنفس الطول تقريبًا (الضغط بيوحّدهم) ونفس
     المقاطع القصيرة دي — فالبصمة تتطابق والكاش يرجّع **صورة قديمة
     لصورة جديدة خالص**. ده كان بيبان كأنه "بيجيب صورة اتولدت قبلها".
     الإصلاح: هاش تراكمي على **كل** بيانات الصورة (كل ٧ حروف، عشان
     يفضل سريع على صور ضخمة) + الطول. أي اختلاف حقيقي في البيكسلات
     بيغيّر الهاش، ونفس الصورة بالظبط بتدي نفس الهاش دايمًا. */
  function faceSig(dataUrl) {
    var s = String(dataUrl || "");
    if (!s) return "";
    // FNV-1a-ish تراكمي — بسيط وسريع وحسّاس لأي تغيير في المحتوى
    ...
    // الطول كمان — طبقة تفريق إضافية رخيصة
  ...
  /* Persistent high-quality result cache. localStorage is too small for several
     2K try-on grids, so generated images live in Cache Storage when available.
     The key still includes product image + customer face + selected colors. */
  var RESULT_CACHE_NAME = "echarpe-tryon-results-v1";
  function _persistentUrl(productId, faceDataUrl, colors, productDataUrl) {
    var k = cacheKey(productId, faceDataUrl, colors, productDataUrl);
    return (typeof location !== "undefined" ? location.origin : "https://echarpe.local") +
      "/__tryon_result_cache__?k=" + encodeURIComponent(k);
  }
  ...
  function _blobToDataUrl(blob) {
    return new Promise(function(resolve){
      ...
      r.onerror=function(){resolve("");}; r.readAsDataURL(blob);
    });
  }
  function readResultPersistent(productId, faceDataUrl, colors, productDataUrl) {
    if (typeof caches === "undefined") return Promise.resolve("");
    var u = _persistentUrl(productId, faceDataUrl, colors, productDataUrl);
    return caches.open(RESULT_CACHE_NAME).then(function(c){ return c.match(u); })
      .then(function(r){ return r ? r.blob() : null; })
      .then(function(b){ return b ? _blobToDataUrl(b) : ""; })
      .catch(function(){ return ""; });
  }
  function saveResultPersistent(productId, faceDataUrl, imageDataUrl, colors, productDataUrl) {
    if (typeof caches === "undefined" || !isImageDataUrl(imageDataUrl)) return Promise.resolve(false);
    var b = _dataUrlToBlob(imageDataUrl); if (!b) return Promise.resolve(false);
    var u = _persistentUrl(productId, faceDataUrl, colors, productDataUrl);
    return caches.open(RESULT_CACHE_NAME).then(function(c){
      return c.put(u, new Response(b, {headers:{"Content-Type":b.type || "image/jpeg"}}));
  ...
  var API = {
    ...
    readResultPersistent: readResultPersistent,
    saveResultPersistent: saveResultPersistent
  };
