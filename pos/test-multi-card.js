...
// ============================================================
// 📴 الشبكة الاحتياطية لازم توصل السيرفر فعلًا
// الشكوى: عملية استنت أكتر من 30 ثانية والكاشير طبعت يدوي في الآخر.
// السبب: enablePersistence شغّالة، و .get() العادية بترد من الكاش المحلي.
// لما الاتصال يتعب: onSnapshot مبيوصلش + poll يقرا كاش فاضي + الخطأ يتبلع
// في catch فاضي → الشبكة الاحتياطية بتفشل في نفس الحالة اللي اتعملت عشانها.
// ============================================================
{
  const wf4 = extractFn(saleSrc, 'paymobWatch');
  assert(/\.get\(\{ source: 'server' \}\)/.test(wf4),
    "🔑 الـpoll بيقرا من السيرفر إجباري — مش من الكاش");
  assert(!/doc\(orderRef\)\.get\(\);/.test(wf4),
    '🔴 القراءة اللي كانت بترد من الكاش اتشالت');

  // الخطأ مابقاش يتبلع في صمت
  assert(!/\}catch\(e\)\{\}\s*\n\s*\}, 4000\);/.test(wf4),
    'الـcatch الفاضي على الـpoll اتشال');
  assert(/_pmPollFails\+\+/.test(wf4), 'فشل الوصول بيتعدّ');
  assert(/_pmPollFails = 0;/.test(wf4), 'والعداد بيتصفّر مع أول نجاح');
  assert(/_pmPollFails === 3/.test(wf4),
    '📴 بعد 3 محاولات (~12 ثانية) الكاشير بتتبلّغ — مش تفضل تبص على شاشة ساكتة');
  assert(/النت مش واصل للسيرفر/.test(wf4), 'برسالة واضحة');
  assert(/دوس «حفظ وطباعة» وأكّد/.test(wf4), 'وبتوجّهها للحفظ اليدوي');

  // العداد بيتصفّر مع كل محاولة دفع جديدة
  const reset = extractFn(saleSrc, 'paymobResetActive');
  assert(/_pmPollFails = 0;/.test(reset),
    'عداد الفشل بيبدأ من الأول مع كل طلب جديد للماكينة');
}
