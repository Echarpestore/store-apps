...
async function sendTransfer(){
  if(typeof confirmForeignBranchAction === 'function' && !(await confirmForeignBranchAction('إرسال التحويلة وخصم البضاعة من المخزون'))) return;
...
async function confirmTransfer(id, confirmer){
  if(typeof confirmForeignBranchAction === 'function' && !(await confirmForeignBranchAction('استلام التحويلة'))) return;
  const t = _trList.find(x=> x.id === id); if(!t) return;
