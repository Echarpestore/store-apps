...
function branchCfg(branch){
  try{
    if(typeof window!=='undefined'&&typeof window.echarpeCctvProfile==='function'){
      var p=window.echarpeCctvProfile(branch || (typeof currentBranch!=='undefined'?currentBranch:''));
      if(p){var cam=(p.cameras||[]).find(function(x){return String(x.id)===String(p.cashierCamera);})||(p.cameras||[])[0];if(!cam)return null;return {id:p.id,camera:cam.name||('CAM'+cam.id),cameraId:String(cam.id),stream:cam.stream,remote:p.gateway,localAgent:p.localAgent,localOnly:!!p.localEvidence,playback:!!p.playback};}
    }
  }catch(e){}
  // Safe fallback while a mixed deployment is loading old/new files.
...
function postLocal(path,payload,ms,meta){
  ...
  var c=cfg(meta),base=(c&&c.localAgent)||LOCAL_AGENT;
  return fetchTimeout(base+path,{method:'POST',body:JSON.stringify(payload||{}),cache:'no-store'},ms||10000).then(function(r){if(!r.ok)throw new Error('agent_http_'+r.status);return r.json();});
}
...
async function grabGlowStage(stage,meta){
  ...
  var j=await postLocal('/echarpe-events/snapshot-stage',{sid:sid,invoiceCode:invoiceCode,stage:stage,camera:Number(c.cameraId)||1,atMs:Number(meta.atMs)||Date.now()},12000,meta);
  return {stage:stage,camera:c.camera,stream:c.stream,capturedAtMs:Number(j.capturedAtMs)||Number(meta.atMs)||Date.now(),nvrAtMs:Number(j.nvrAtMs)||0,nvrOffsetMs:Number(j.nvrOffsetMs)||0,clockSource:String(j.clockSource||''),bytes:Number(j.bytes)||0,available:true,localOnly:true};
}
function captureStage(stage,meta){
  try{
    ...
    if(stage!=='first_item'&&typeof cctvBasketTimelineObserve==='function')try{cctvBasketTimelineObserve({sid:sid,branch:meta.branch,atMs:Number(meta.atMs)||Date.now(),kind:stage,force:true});}catch(e){}
    ...
    var c=cfg(meta),fn=(c&&c.localOnly)?grabGlowStage:grabLegacyShot;
...
function publicLocalShot(shot){return shot?{stage:shot.stage,camera:shot.camera||'CAM1',stream:shot.stream||'',capturedAtMs:Number(shot.capturedAtMs)||Date.now(),nvrAtMs:Number(shot.nvrAtMs)||0,nvrOffsetMs:Number(shot.nvrOffsetMs)||0,clockSource:String(shot.clockSource||''),bytes:Number(shot.bytes)||0,available:true}:null;}
...
async function finalizeGlow(meta,sid){
  ...
  try{
    var c=cfg(meta),fin=await postLocal('/echarpe-events/snapshot-finalize',{sid:sid,invoiceCode:String(meta.invoiceCode)},7000,meta);
    if(fin&&fin.shots)Object.keys(fin.shots).forEach(function(k){if(STAGES[k])shots[k]=Object.assign({},shots[k]||{},fin.shots[k],{stage:k,camera:c.camera,stream:c.stream,available:true,localOnly:true});});
...
async function snapshot(meta){
  try{meta=meta||{};var c=cfg(meta);if(!c||!meta.invoiceCode||typeof db==='undefined')return false;if(c.localOnly){var shot=await grabGlowStage('after_save',{invoiceCode:String(meta.invoiceCode),branch:meta.branch||'',atMs:Number(meta.atMs)||Date.now()});var s={after_save:shot};return await writeInvoiceDoc(meta,s,true);}var legacy=await grabLegacyShot('after_save',meta);return await writeInvoiceDoc(meta,{after_save:legacy},false);}catch(e){try{console.warn('CCTV invoice snapshot skipped',e&&e.message||e);}catch(_){}return false;}
}
function postAgent(payload,meta){var body=JSON.stringify(payload),localOpt={method:'POST',body:body,cache:'no-store'},remoteOpt={method:'POST',headers:{'Content-Type':'application/json'},body:body,cache:'no-store'},c=cfg(meta),base=(c&&c.localAgent)||LOCAL_AGENT;return fetchTimeout(base+'/echarpe-events/event',localOpt,8000).catch(function(){if(!c||!c.remote)throw new Error('remote_agent_not_configured');return fetchTimeout(c.remote+'/echarpe-events/event',remoteOpt,10000);});}
async function registerEvent(meta){
  try{
    ...
    // Local-agent invoice video is NVR-on-demand; do not store a file per sale.
    if(c.localOnly&&meta.type==='sale_saved')return true;
    var id=safeId(meta.eventId),view=c.remote?(c.remote+'/echarpe-events/view?id='+encodeURIComponent(id)):'';
    var doc={eventId:id,type:meta.type||'event',branch:meta.branch||'',camera:c.camera,stream:c.stream,atMs:Number(meta.atMs)||Date.now(),beforeSec:30,afterSec:30,invoiceCode:meta.invoiceCode||'',invoiceNo:meta.invoiceNo||'',sid:meta.sid||'',employeeId:meta.employeeId||'',employeeName:meta.employeeName||'',transactionKind:meta.transactionKind||'',viewerUrl:view,storage:'branch_local',version:555};
    db.collection(EVENT_COL).doc(id).set(doc,{merge:true}).catch(function(){});postAgent({id:id,type:doc.type,invoiceCode:doc.invoiceCode,invoiceNo:doc.invoiceNo,transactionKind:doc.transactionKind,branch:doc.branch,atMs:doc.atMs},meta).catch(function(){});return true;
  }catch(e){try{console.warn('CCTV event skipped',e&&e.message||e);}catch(_){}return false;}
}
function invoiceEvidence(meta){try{if(!meta||!meta.invoiceCode)return false;if(typeof cctvFinalizeBasketTimeline==='function')cctvFinalizeBasketTimeline(meta);finalizeInvoice(meta);if(typeof cctvPresenceRecordSale==='function')cctvPresenceRecordSale(meta);registerEvent({eventId:meta.eventId||invoiceEventId(meta.invoiceCode),type:'sale_saved',branch:meta.branch||'',atMs:Number(meta.atMs)||Date.now(),invoiceCode:meta.invoiceCode,invoiceNo:meta.invoiceNo||'',sid:meta.sid||meta.cartSid||'',employeeId:meta.employeeId||'',employeeName:meta.employeeName||'',transactionKind:meta.transactionKind||'sale'});return true;}catch(e){return false;}}
