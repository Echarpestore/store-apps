...
const OF_ACT_KINDS = {
  ...
  credit_otp_ok:         { t:'🔐 رصيد اتصرف بكود العميلة', g:'money' },
  ...
  credit_otp_failed:     { t:'🔐 كود رصيد فشل (غلط/انتهى)', g:'money', hot:true },
  credit_otp_no_app:     { t:'📵 رصيد متصرفش — العميلة معندهاش التطبيق', g:'money' },
  ...
  paymob_presave_amount_mismatch: { t:'📟 مبلغ الكارت مش مطابق للفاتورة', g:'money', hot:true },
  paymob_txn_id_reconciled:       { t:'📟 رقم عملية الكارت اتحدّث', g:'money' },
  ...
  item_qty_reduced:      { t:'🛒 كمية صنف اتقللت', g:'cart' },
  item_removed_late:     { t:'🛒⏱️ صنف اتشال بعد وقت طويل في السلة', g:'cart', hot:true },
...
function ofActIntelligence(a){
  ...
  if(type === 'credit_spend_failed') return watch(
    'فاتورة اتقفلت وفيها خصم من رصيد العميلة، بس السيرفر رفض يخصم المبلغ من حسابها. يعني العميلة خدت الخصم ورصيدها زي ما هو.',
    'دي خسارة مباشرة عليك، والعميلة تقدر تستخدم نفس الرصيد تاني. السبب مكتوب في «الرسالة».',
    'اخصم المبلغ يدوي من رصيد العميلة (تعديل الرصيد) بنفس رقم الفاتورة، وصلّح السبب: لو «مش موظف» = جهاز الكاشير مش داخل بحسابه · لو رسالة قديمة = دالة creditSpend محتاجة تتنشر.');
  if(type === 'credit_spend_blocked') return watch(
    'الكاشير حاولت تستخدم رصيد عميلة، والسيستم سأل السيرفر الأول فلقاه مش جاهز — فمنع الخصم قبل ما يتحط على الفاتورة.',
    'ده الحارس اللي بيمنع «فاتورة بخصم ورصيد مااتخصمش». مفيش خسارة حصلت، بس الرصيد مش شغال في الفرع ده لحد ما السبب يتصلّح.',
    'اقرا «الرسالة»: صلاحية = جهاز الكاشير محتاج يدخل بحساب الموظف · غير كده = انشر دالة creditSpend.');
  if(type === 'credit_otp_failed') return watch(
    'كود تأكيد صرف الرصيد اتكتب غلط 3 مرات أو انتهى وقته، فالصرف متمّش.',
    'مرة واحدة = عميلة غلطت في رقم. لو بيتكرر عند نفس الموظفة أو على نفس رقم العميلة، ممكن حد بيجرّب يخمّن الكود.',
    'بصّ على رقم العميلة والموظفة والتكرار. الكود بيموت بعد 3 محاولات، فالتخمين مبيوصلش لحاجة — بس التكرار علامة.');
  if(type === 'reward_sent') return watch(
    'موظف بعت مكافأة (خصم مؤجّل) لعميلة أو لمجموعة عملاء من الكاشير.',
    'المكافأة بتتخصم من فاتورة جاية لحد قيمتها كاملة — يعني مكافأة كبيرة بحد أدنى صغير = بضاعة شبه مجانية. قبل v714 ده كان بيحصل من غير أي أثر.',
    'بصّ على القيمة والحد الأدنى والعدد والأرقام. مكافأة لرقم واحد بقيمة كبيرة من موظف مش معتاد يبعت = اسأل. ولو الرقم بتاع حد من الفريق أو قرايبه، راجع فواتير الرقم ده بعدها.');
  if(type === 'paymob_presave_rejected_attempt') return watch(
    'الكاشير حاولت تحفظ فاتورة بفيزا، وPaymob نفسه بيقول إن العملية دي فشلت أو اتلغت أو اترجعت — فالسيستم منع الحفظ.',
    'لو كانت اتحفظت كانت هتطلع عجز في التقفيل: فاتورة مدفوعة على الورق ومفيش فلوس دخلت.',
    'اتأكد إن العميلة دفعت بطريقة تانية أو إن الفيزا اتعادت بنجاح. لو الحدث بيتكرر مع نفس الموظفة، راجع إيصالات الماكينة.');
  if(type === 'paymob_presave_amount_mismatch') return watch(
    'المبلغ اللي Paymob سجّله على العملية مختلف عن مبلغ شريحة الكارت في الفاتورة — فالسيستم منع الربط بينهم.',
    'ده معناه إما عملية قديمة بتتربط بفاتورة جديدة، أو مبلغ اتكتب غلط على الماكينة. الفلوس اتسحبت فعلًا بالمبلغ اللي عند Paymob.',
    'طابق «المتوقع» و«الفعلي» في التفاصيل مع إيصال الماكينة، وشوف الفاتورة اتقفلت إزاي بعدها.');
  ...
  if(type === 'item_qty_reduced') return normal(
    'كمية صنف في السلة اتقللت قبل إتمام البيع (مثلًا 3 قطع بقوا 1).',
    'غالبًا العميلة غيّرت رأيها أو الكاشير مسحت زيادة بالغلط — وده بيحصل بعد ثواني من المسح.',
    'مش محتاج مراجعة إلا لو بيتكرر كتير عند نفس الموظفة.');
  if(type === 'item_removed_late') return watch(
    'صنف اتشال (أو كميته اتقللت) بعد ما قعد في السلة دقيقة ونص أو أكتر.',
    'المسحة الغلط بتتصلّح في ثواني. صنف يتشال بعد وقت طويل ممكن يكون عميلة غيّرت رأيها عند الدفع — وممكن يكون صنف اتمسح قدام العميلة واتشال بعد ما دفعت كاش ومشيت.',
    'بصّ على «قعد في السلة» و«قيمة اللي اتشال» و«الطريقة»، وافتح قصة السلة: الفاتورة اتحفظت بكام؟ والدفع كان كاش؟ لو عندك كاميرا، ده الوقت اللي تراجعه. حدث واحد مش دليل — التكرار عند نفس الموظفة هو اللي يتبصّ عليه.');
...
const OF_ACT_FIELD_AR = {
  ...
  reason:'السبب', cause:'سبب الفرق', causeSum:'مجموع السبب', causeExact:'السبب مطابق للفرق', count:'العدد', itemCount:'عدد القطع', cartCountAfter:'باقي في السلة', rewardType:'نوع المكافأة', minInvoice:'الحد الأدنى للفاتورة', days:'صالحة (يوم)', bulk:'إرسال جماعي', phones:'الأرقام', maxPossibleEGP:'أقصى تكلفة (ج.م)', rewardId:'رقم المكافأة', inCartSec:'قعد في السلة (ثانية)', value:'قيمة اللي اتشال', how:'الطريقة', qtyBefore:'الكمية قبل', qtyAfter:'الكمية بعد', isReturn:'سطر مرتجع',
