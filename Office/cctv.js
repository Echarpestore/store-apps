...
(function(){
  ...
  async function openDayBasketPlayback(){
    ...
    if(master){
      ...
      var glowGapFallbackUsed={};master.addEventListener('error',function(){if(chunkRetry<2&&document.documentElement.contains(ov)){chunkRetry++;status.style.display='block';status.textContent='إعادة الاتصال بالتسجيل تلقائيًا ('+chunkRetry+'/2)…';if(master._retryTimer)clearTimeout(master._retryTimer);master._retryTimer=setTimeout(function(){loadChunk(chunkStart,true);},2200);return;}if(glowDay&&document.documentElement.contains(ov)){var failedKey=String(Math.floor(chunkStart/60000));if(!glowGapFallbackUsed[failedKey]){glowGapFallbackUsed[failedKey]=1;var nextEvent=jumpEvents.find(function(e){return Number(e.atMs)>chunkStart+60000;}),fallbackAt=nextEvent?Math.max(coverageStart,Number(nextEvent.atMs)-10000):Math.max(coverageStart,coverageEnd-90000);if(Math.abs(fallbackAt-chunkStart)>=30000){status.style.display='block';status.textContent='لا يوجد تسجيل في هذه الدقيقة — الانتقال تلقائيًا لأقرب جزء متاح…';setTimeout(function(){loadChunk(fallbackAt);},700);return;}}}status.style.display='block';status.textContent='التسجيل غير متاح في هذا التوقيت.';});
