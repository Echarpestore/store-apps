...
(function(){
  ...
  var _busyTimer=0;
  function setCctvBusy(on,label,progress){
    var box=document.getElementById('ofCctvBusy');if(!box)return;
    if(_busyTimer){clearTimeout(_busyTimer);_busyTimer=0;}
    box.classList.toggle('show',!!on);
    var txt=box.querySelector('[data-cctv-busy-text]'),bar=box.querySelector('[data-cctv-busy-bar]');
    if(txt)txt.textContent=label||'جاري التحميل…';
    if(bar){
      var p=Number(progress);
      bar.style.width=isFinite(p)&&p>=0?Math.max(8,Math.min(100,p))+'%':'38%';
      bar.classList.toggle('indeterminate',!(isFinite(p)&&p>=0));
    }
    if(!on){_busyTimer=setTimeout(function(){box.classList.remove('show');},180);}
  }
  ...
  function openLocalReview(){if(LOCAL_REVIEW){location.href='/Office/';return;}var url='http://192.168.0.107:1990/Office/';window.location.href=url;}
  ...
  function armSnapshotFrame(img){
    if(!img)return;var base=img.getAttribute('data-live-frame'),branchId=img.getAttribute('data-live-branch')||'',delay=branchId==='glow'?160:900;
    function next(ms){if(img._frameTimer)clearTimeout(img._frameTimer);img._frameTimer=setTimeout(load,ms);}
    function load(){
      if(!state.active||document.hidden||!document.documentElement.contains(img))return;
      if(branchId==='glow'){
        var pre=new Image();
        pre.onload=function(){if(!document.documentElement.contains(img))return;img.src=pre.src;img.classList.add('is-ready');next(delay);};
        pre.onerror=function(){next(700);};
        pre.src=base+Date.now();return;
      }
      img.onload=function(){img.classList.add('is-ready');next(delay);};img.onerror=function(){img.classList.remove('is-ready');next(2500);};img.src=base+Date.now();
    }
    load();
  }
  ...
  function cameraCard(c,i){
    ...
    var liveMedia=isGlow?'<img title="'+esc(c.name)+'" data-live-frame="'+esc(frameUrl(c))+'" data-live-branch="glow" alt="'+esc(c.name)+' live" loading="eager">':(useMse?'<iframe title="'+esc(c.name)+'" data-stream-src="'+esc(streamUrl(c))+'" src="'+esc(streamUrl(c))+'" allow="autoplay; fullscreen" allowfullscreen loading="eager"></iframe>':'<img title="'+esc(c.name)+'" data-live-frame="'+esc(frameUrl(c))+'" data-live-branch="'+esc(b().id)+'" alt="'+esc(c.name)+' live" loading="eager">');
  ...
  function playbackDefaultMinutes(x){return 1;}
  ...
  function armPlaybackBackGuard(){
    try{
      if(history.state&&history.state.ofCctvPlaybackV591)return;
      history.pushState(Object.assign({},history.state||{},{ofCctvPlaybackV591:true}),'',location.href);
    }catch(e){}
  }
  function closePlaybackModalByUser(){
    var had=closePlaybackModal();
    if(!had)return;
    try{
      if(history.state&&history.state.ofCctvPlaybackV591)history.back();
    }catch(e){}
  }
  async function openPlayback(atMs,durationMin,cameraId,offsetMs){
    var x=b(),mins=Math.max(1,Number(durationMin)||playbackDefaultMinutes(x));
    atMs=await normalizePlaybackStart(x,atMs,mins,cameraId);
    durationMin=mins;
    ...
    ov.innerHTML='<div class="of-cctv-playback-modal"><div class="of-cctv-playback-head"><b>🎞️ مراجعة التسجيل · '+esc(x.name)+'</b><div>'+(videoOnly?'<span class="of-cctv-playback-noaudio">🔇 بدون صوت</span>':'<button type="button" data-pb-sound class="of-cctv-playback-close">🔊 تشغيل الصوت</button>')+(isGlow?'':(x.id==='madinaty'?'<select data-pb-quality aria-label="جودة التسجيل"><option value="360" selected>360p سريع</option><option value="480">480p</option><option value="720">720p</option></select>':'<select data-pb-quality aria-label="جودة التسجيل"><option value="480" selected>480p سريع</option><option value="720">720p</option></select>'))+(isGlow?'<button type="button" class="of-cctv-playback-close" data-pb-latest>آخر تسجيل</button>':'')+'<a class="of-cctv-playback-external" href="'+esc(u)+'" target="_blank" rel="noopener">فتح منفصل ↗</a><button type="button" class="of-cctv-playback-close" data-pb-close aria-label="إغلاق">✕</button></div></div><video controls autoplay playsinline src="'+esc(u)+'"></video><div class="of-pb665-loader" data-pb-status><div class="of-pb665-loader-top"><span data-pb-status-text>جاري تجهيز التسجيل…</span><b>VIDEO</b></div><div class="of-pb665-loader-track"><i data-pb-progress></i></div></div><div style="display:flex;gap:8px;align-items:center;justify-content:center;padding:9px;background:#111827;color:#fff"><button type="button" data-pb-prev class="of-cctv-playback-close">⏮ السابق</button><b data-pb-clock></b><button type="button" data-pb-next class="of-cctv-playback-close">التالي ⏭</button></div></div>';
    document.body.appendChild(ov);armPlaybackBackGuard();
    var start=Number(atMs)||Date.now(),step=(Number(durationMin)||playbackDefaultMinutes(x))*60000,video=ov.querySelector('video'),clock=ov.querySelector('[data-pb-clock]'),ext=ov.querySelector('.of-cctv-playback-external'),quality=ov.querySelector('[data-pb-quality]'),pbStatus=ov.querySelector('[data-pb-status]'),pbStatusText=ov.querySelector('[data-pb-status-text]'),pbProgress=ov.querySelector('[data-pb-progress]'),pbRetry=0;
    function pbLoading(text,pct){if(pbStatus)pbStatus.classList.add('show');if(pbStatusText)pbStatusText.textContent=text||'جاري تجهيز التسجيل…';if(pbProgress){pbProgress.style.width=(pct||36)+'%';pbProgress.classList.add('indeterminate');}}
    function pbReady(){if(pbProgress){pbProgress.classList.remove('indeterminate');pbProgress.style.width='100%';}if(pbStatusText)pbStatusText.textContent='جاهز';setTimeout(function(){if(pbStatus)pbStatus.classList.remove('show');},260);}
    async function loadAt(next,resumeSec,isRetry){
      ...
      ext.href=nextUrl;pbLoading(pbRetry?'إعادة الاتصال بالتسجيل ('+pbRetry+'/3)…':'جاري تجهيز التسجيل…',pbRetry?58:34);
    ...
    ov.onclick=function(e){if(e.target===ov)closePlaybackModalByUser();};
  }
  function syncPlaybackPanel(){
    ...
    var hint=document.getElementById('ofCctvNvrHint');if(hint)hint.textContent=b().playback?(b().id==='rehab'?'تسجيل الكاشير فقط على كمبيوتر الفرع · باقي الكاميرات للمشاهدة المباشرة':b().id==='madinaty'?'تسجيل كمبيوتر الفرع — دقيقة سريعة Video-only.':b().id==='glow'?'تسجيل كمبيوتر Glow — دقيقة سريعة Video-only، والوقت يتظبط تلقائيًا على آخر تسجيل متاح.':'اختار الكاميرا واليوم والوقت.'):'';
  ...
  function openPlaybackFromControls(){
    ...
    openPlayback(dt.getTime(),Number(du&&du.value)||1,String(ca&&ca.value||b().playbackCamera||'1'));
  }
  ...
  function renderAllBranchesLive(){
    ...
    grid.innerHTML=BRANCHES.map(function(x){var c=(x.cameras||[]).find(function(q){return String(q.id)===String(x.playbackCamera);})||x.cameras[0],useMse=!!c.liveStream||x.id!=='madinaty',media=x.id==='glow'?'<img title="'+esc(x.name)+'" data-live-frame="'+esc(frameUrlFor(x,c))+'" data-live-branch="glow" alt="'+esc(x.name)+' live" loading="eager">':(useMse?'<iframe title="'+esc(x.name)+'" data-stream-src="'+esc(streamUrlFor(x,c))+'" src="'+esc(streamUrlFor(x,c))+'" allow="autoplay; fullscreen" allowfullscreen loading="eager"></iframe>':'<img data-live-frame="'+esc(frameUrlFor(x,c))+'" data-live-branch="'+esc(x.id)+'" alt="'+esc(x.name)+' live">');return '<article class="of-cctv-all-card"><div class="of-cctv-all-head"><div><b>'+esc(x.name)+'</b><small>'+esc(c.label||c.name)+'</small></div><div><button type="button" data-all-branch="'+esc(x.id)+'">كل الكاميرات</button><button type="button" data-all-playback="'+esc(x.id)+'">🎞 تسجيل</button></div></div><div class="of-cctv-all-media">'+media+'</div></article>';}).join('');
  ...
  async function openDayBasketPlayback(){
    ...
    var videosHtml=glowDay
      ? '<div class="of-day596-glow-wrap"><span class="of-smart555-label">كاميرا '+esc(primaryCam)+' · Glow</span><video data-day-master controls autoplay playsinline></video></div>'
      : dualCam
    ...
    ov.innerHTML='<div class="of-cctv-playback-modal"><div class="of-cctv-playback-head"><div><b>🎬 اليوم الكامل + السلة</b><small class="of-day511-muted" data-day-coverage></small></div><div class="of-day564-head-actions"><span class="of-day592-noaudio">🔇 بدون صوت</span><button type="button" data-sync-minus>−1ث</button><button type="button" data-sync-zero>0ث</button><button type="button" data-sync-plus>+1ث</button><button type="button" class="of-cctv-playback-close" data-pb-close>✕</button></div></div><div class="of-day511-body"><div class="of-day511-stage">'+videosHtml+'<div class="of-day665-timeline"><div class="of-day665-timeline-head"><b data-day-seek-label>اسحب لاختيار الوقت</b><span>الخط الزمني للتسجيل</span></div><input type="range" data-day-slider step="5" aria-label="الانتقال داخل تسجيل اليوم"><div class="of-day665-marker-track" data-day-markers></div></div><div class="of-day665-events-head"><b>أحداث السلة</b><span>اضغط على أي حدث للانتقال مباشرة</span></div><div class="of-day564-event-rail" data-day-event-rail></div><div class="of-day666-buffer"><div><i data-day-buffer></i></div><span data-day-buffer-pct>0% محمّل</span></div><div class="of-day511-nav of-day666-nav"><button type="button" data-day-prev>−10 ث</button><button type="button" data-day-next>+10 ث</button></div></div><aside class="of-day511-cart"><div class="of-day511-carthead"><b>🛒 السلة في هذه اللحظة</b><div class="of-day511-muted" data-day-clock>—</div><div class="of-day511-event" data-day-event>بين الفواتير</div><div class="of-day511-muted" data-day-sync>تزامن السلة: 0 ثانية</div><div class="of-day665-load" data-day-status><span data-day-status-text>جاري تجهيز التسجيل…</span><div><i data-day-progress></i></div></div></div><div class="of-day511-rows" data-day-rows></div><div class="of-day511-total"><span>الإجمالي</span><strong data-day-total>0.00 ج.م</strong></div></aside></div></div>';
    closePlaybackModal();document.body.appendChild(ov);armPlaybackBackGuard();setCctvBusy(false,'جاهز',100);
    var master=ov.querySelector('[data-day-master]'),slave=dualCam?ov.querySelector('[data-day-slave]'):null;
    var slider=ov.querySelector('[data-day-slider]'),marks=ov.querySelector('[data-day-markers]'),rail=ov.querySelector('[data-day-event-rail]'),clock=ov.querySelector('[data-day-clock]'),eventEl=ov.querySelector('[data-day-event]'),rowsEl=ov.querySelector('[data-day-rows]'),totalEl=ov.querySelector('[data-day-total]'),status=ov.querySelector('[data-day-status]'),statusText=ov.querySelector('[data-day-status-text]'),dayProgress=ov.querySelector('[data-day-progress]'),seekLabel=ov.querySelector('[data-day-seek-label]'),syncText=ov.querySelector('[data-day-sync]');
    function dayLoading(text,pct){if(status)status.classList.add('show');if(statusText)statusText.textContent=text||'جاري تجهيز التسجيل…';if(dayProgress){dayProgress.style.width=(pct||34)+'%';dayProgress.classList.add('indeterminate');}}
    function updateBufferedProgress(){
      if(!master||!master.duration||!isFinite(master.duration)||!master.buffered||!master.buffered.length)return;
      var end=0;for(var bi=0;bi<master.buffered.length;bi++)end=Math.max(end,Number(master.buffered.end(bi))||0);
      var pct=Math.max(0,Math.min(100,Math.round(end/master.duration*100)));
      if(status)status.classList.add('show');
      if(dayProgress){dayProgress.classList.remove('indeterminate');dayProgress.style.width=pct+'%';}
      if(statusText)statusText.textContent='تم تحميل '+pct+'% من المقطع الحالي';
      var buf=ov.querySelector('[data-day-buffer]');if(buf)buf.style.width=pct+'%';
      var bp=ov.querySelector('[data-day-buffer-pct]');if(bp)bp.textContent=pct+'% محمّل';
    }
    ...
    if(master){
      ...
      master.addEventListener('loadedmetadata',function(){dayLoading('تم تجهيز المقطع — جاري بدء التشغيل…',76);});master.addEventListener('playing',function(){chunkRetry=0;dayReady();});master.addEventListener('waiting',function(){dayLoading('جاري تحميل الفيديو…',67);});
      master.addEventListener('progress',updateBufferedProgress);master.addEventListener('canplay',updateBufferedProgress);master.addEventListener('timeupdate',function(){renderAt();updateBufferedProgress();});master.addEventListener('seeking',function(){renderAt();});
      ...
      var glowGapFallbackUsed={};master.addEventListener('error',function(){if(chunkRetry<2&&document.documentElement.contains(ov)){chunkRetry++;dayLoading('إعادة الاتصال بالتسجيل تلقائيًا ('+chunkRetry+'/2)…',58);if(master._retryTimer)clearTimeout(master._retryTimer);master._retryTimer=setTimeout(function(){loadChunk(chunkStart,true);},2200);return;}if(glowDay&&document.documentElement.contains(ov)){var failedKey=String(Math.floor(chunkStart/60000));if(!glowGapFallbackUsed[failedKey]){glowGapFallbackUsed[failedKey]=1;var nextEvent=jumpEvents.find(function(e){return Number(e.atMs)>chunkStart+60000;}),fallbackAt=nextEvent?Math.max(coverageStart,Number(nextEvent.atMs)-10000):Math.max(coverageStart,coverageEnd-90000);if(Math.abs(fallbackAt-chunkStart)>=30000){dayLoading('لا يوجد تسجيل هنا — الانتقال لأقرب جزء متاح…',62);setTimeout(function(){loadChunk(fallbackAt);},700);return;}}}dayLoading('التسجيل غير متاح في هذا التوقيت.',100);if(dayProgress)dayProgress.classList.remove('indeterminate');});
    ...
    function sliderTargetMs(){return dayReviewBounds.start+Number(slider.value)*1000;}
    function previewSlider(){sliderDragging=true;var t=sliderTargetMs();if(seekLabel)seekLabel.textContent='الانتقال إلى '+new Date(t).toLocaleTimeString('ar-EG',{hour:'2-digit',minute:'2-digit',second:'2-digit'});clock.textContent=new Date(t).toLocaleString('ar-EG');}
    var lastSliderSeek=0;function commitSlider(){var t=sliderTargetMs();sliderDragging=false;if(Math.abs(t-lastSliderSeek)<1000)return;lastSliderSeek=t;seekToMs(t);}
    slider.addEventListener('input',previewSlider);slider.addEventListener('change',commitSlider);slider.addEventListener('pointerup',commitSlider);slider.addEventListener('touchend',commitSlider,{passive:true});slider.addEventListener('keyup',function(e){if(['ArrowLeft','ArrowRight','Home','End','PageUp','PageDown','Enter',' '].indexOf(e.key)>=0)commitSlider();});ov.querySelector('[data-day-prev]').onclick=function(){seekToMs(Math.max(coverageStart,currentVideoNow-10000));};ov.querySelector('[data-day-next]').onclick=function(){seekToMs(Math.min(coverageEnd-1000,currentVideoNow+10000));};var closeBtn=ov.querySelector('[data-pb-close]');if(closeBtn)closeBtn.onclick=closePlaybackModalByUser;ov.onclick=function(e){if(e.target===ov)closePlaybackModalByUser();};loadChunk(chunkStart);
  }
  ...
  async function loadDayReview(){
    ...
    if(btn){btn.disabled=true;btn.textContent='جاري التحميل…';} setCctvBusy(true,'جاري تحميل فواتير اليوم…',25);box.innerHTML='<div class="of-cctv-day-empty"><div class="of-inline665-loader"><i></i></div><b>جاري تحميل اليوم…</b><small>بنجيب الفواتير ونجهزها للمراجعة</small></div>';
    try{
      ...
      dayReviewRows=rows.slice();renderDayRows(rows);if(playBtn)playBtn.disabled=false;setCctvBusy(false,'تم تحميل اليوم',100);
    }catch(e){setCctvBusy(false);box.innerHTML='<div class="of-cctv-day-empty">تعذر تحميل مراجعة اليوم: '+esc(e&&e.message||e)+'</div>';}
  ...
  function normalizeTrackingStatus(raw){
    ...
    if(raw.generatedAtMs&&raw.configuredCameras)return {detector:true,state:raw};
    return raw;
  }
  ...
  function renderTrafficReport(det,salesByDay){
    ...
    if(!history.length){var tp0=trackingProfile(b());host.innerHTML='<div class="of-cctv-day-empty">تقرير الزوار يبدأ بعد أول حركة دخول/خروج في '+esc(b().name)+' عبر Camera '+esc(tp0.door)+'.</div>';return;}
    ...
    host.innerHTML='<div class="of-traffic606-cards">'
      ...
      +'</div>'
      +'<div class="of-traffic606-note">'+esc(trackingProfile(b()).label)+'</div>'
  ...
  function init(){
    ...
    document.addEventListener('keydown',function(e){if(e.key==='Escape'&&document.getElementById('ofCctvPlaybackOv')){closePlaybackModalByUser();return;}if(e.key==='Escape'&&(document.querySelector('.of-cctv-room-focus')||document.querySelector('.of-cctv-panel-focus'))){clearFocusClasses();exitNativeFs();}});
    window.addEventListener('office-pos-live-update',refreshPosOnly);window.addEventListener('beforeunload',stop);
    window.addEventListener('popstate',function(){if(document.getElementById('ofCctvPlaybackOv'))closePlaybackModal();});
...
window.ofCctvInvoiceShot = async function(invoiceCode){
  try{
    ...
    var d=snap.data()||{},shots=d.shots||{},localGlow=(!!d.localSnapshots||String(d.storage||'').toLowerCase()==='branch_local')&&(String(d.branchProfile||'').toLowerCase()==='glow'||String(d.branch||'').toLowerCase().indexOf('glow')>=0||String(d.gateway||'').toLowerCase().indexOf('cctv-glow')>=0);
...
window.ofCctvInvoiceShot = async function(invoiceCode){
  try{
    ...
    var order=['first_item','payment','saving','after_save'];
    var labels={first_item:'1️⃣ أول كود',payment:'2️⃣ أثناء الدفع',saving:'3️⃣ أثناء الحفظ',after_save:'4️⃣ بعد الحفظ'};
    var valid=localBranch?order.slice():order.filter(function(k){var x=shots[k]||{};return /^data:image\/jpeg;base64,/.test(String(x.jpegData||''));});
