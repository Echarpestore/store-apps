...
(function(){
  ...
  assert(sw.indexOf("echarpe-tryon-v24") > -1 && appC.indexOf("'v24'") > -1,
...
(function(){
  ...
  assert(ci.indexOf('if(!S.plainUser){') > -1
      && ci.indexOf('S.plain = dom.confidence >= 0.4;') > -1,
    '🔴 تلقائي: لون غالب جدًا = ساده — بس الاختيار اليدوي عمره ما يتداس');

  const sr = extractFn(code, 'function buildStyleRow()');
  assert(sr.indexOf('S.plainUser = true;') > -1,
    'الضغط اليدوي بيسكّت التلقائي');
  assert(sr.indexOf("draw(S.stillResult, S.stillImg)") > -1,
    'والتبديل بيحدّث الصورة فورًا');
  assert(html.indexOf('id="styleRow"') > -1, 'والصف في الصفحة');
  assert(code.indexOf('plain: false, plainUser: false,') > -1,
    'الافتراضي: بالكنار (القالب زي ما هو) لحد ما صورة المنتج أو العميلة يقولوا غير كده');
})();
