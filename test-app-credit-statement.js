...
function run(html, mode) {
  ...
  const DOCS = [{ at: 1, amount: 350 }, { at: 9, amount: -25 }, { at: 5, amount: 10 }].map(x => ({ data: () => x }));
  const q = ordered => ({
...
(async () => {
  for (const app of ['loyalty', 'glow']) {
    ...
    console.log('\n📱 ' + app);
    const A = await run(html, 'noindex');
    ...
    t('⭐ الزرار المكرر بيظهر بس لو الرصيد صفر وفيه حركات', () => {
      ...
      ok(/\(!\(\(Number\(currentCustomer\.credit\)\|\|0\) > 0\) && _creditRows\.length/.test(code.slice(i - 300, i)), 'الشرط');
    });
  }
  t('الكاش اترفع', () => { ok(+fs.readFileSync(path.join(ROOT, 'loyalty/sw.js'), 'utf8').match(/loyalty-shell-v(\d+)/)[1] >= 691); ok(+fs.readFileSync(path.join(ROOT, 'glow/sw.js'), 'utf8').match(/glow-loyalty-v(\d+)/)[1] >= 76); });
